...
describe('components/atoms/FilmInfoSection', () => {
	const props = {
		...
		btnValues: ['детали о фильме', 'свернуть детали'],
		languages: ['Русский', 'Английский'],
		assessment: 1212,
		extra: 'Интересный сюжет',
	};
	it('should toggle btn values ', () => {
		const { getByText } = render(<FilmInfoSection {...props} />);
		let btn = getByText('детали о фильме');
		expect(btn).toBeInTheDocument();
		fireEvent.click(btn);
		btn = getByText('свернуть детали');
		expect(btn).toBeInTheDocument();
		expect(btn.tagName).toBe('BUTTON');
	});
	it('should render correct data', () => {
		const { getByText } = render(<FilmInfoSection {...props} />);
		const heading = getByText('Шоу Патрика Стара (1 сезон)');
		expect(heading).toBeInTheDocument();
		expect(heading.tagName).toBe('H1');
		expect(heading).toHaveClass('FilmInfoSection__heading');
		const year = getByText('2021');
		expect(year).toBeInTheDocument();
		expect(year.tagName).toBe('SPAN');
		expect(year).toHaveClass('FilmInfoSection__commonContainer__seasonIntro');
		const duration = getByText('1 сезон');
		expect(duration).toBeInTheDocument();
		expect(duration.tagName).toBe('SPAN');
		expect(duration).toHaveClass('FilmInfoSection__commonContainer__seasonIntro');
		expect(duration.parentElement).toHaveClass('FilmInfoSection__commonContainer');
		const yearOld = getByText('6+');
		expect(yearOld).toBeInTheDocument();
		expect(yearOld).toHaveClass('FilmInfoSection__commonContainer__seasonIntro');
		expect(yearOld.tagName).toBe('SPAN');
		const param = getByText('США');
		expect(param.tagName).toBe('LI');
		expect(param.parentElement?.tagName).toBe('UL');
		expect(param.parentElement).toHaveClass('FilmInfoSection__commonContainer');
	});
});
